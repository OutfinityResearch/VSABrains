# DS002 - Implementation Plan (Index)

**Status:** Draft  
**Version:** 0.3  
**Last Updated:** 2026-01-26

---

DS002 is split into two smaller documents:

- [DS002a - API Reference](DS002a-API-Reference.md)
- [DS002b - Implementation Guide](DS002b-Implementation-Guide.md)

Source of truth:

- DS004: definitive runtime semantics (step ordering, displacement, localization, replay, checkpoints)
- DS005: non-core integrations (text ingestion, LLM extraction, validation, retrieval, conflict rules, derivation, answer contract)

Recommended reading:

1. DS001 (Architecture + Glossary)
2. DS004 (Core algorithms)
3. DS002b (Implementation guide)
4. DS002a (API reference while coding)
5. DS003 (Evaluation)
6. DS005 (Integrations, for Exp3)
