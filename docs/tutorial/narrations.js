...
// Cache for loaded narrations and per-scene metadata (e.g. key ideas).
...
/**
 * Get the "Key idea" for a scene in a specific language.
 * Unlike narration, we keep this silent (no warnings) because the UI may poll it frequently.
 * @param {string} sceneId
 * @param {string} lang
 * @returns {string} Key idea text or empty string
 */
export function getKeyIdea(sceneId, lang = 'en') {
  if (!narrationCache) return '';
  const sceneEntry = narrationCache[sceneId];
  if (!sceneEntry) return '';
  const keyIdeas = sceneEntry.keyIdeas || {};

  const normalizedLang = String(lang || 'en').toLowerCase().trim();
  const shortLang = normalizedLang.split('-')[0];
  return keyIdeas[lang] || keyIdeas[normalizedLang] || keyIdeas[shortLang] || '';
}
